...
import osepcafe.FrameLogin.User;
...
public class FrameTransaksi extends javax.swing.JFrame {
    ...
    class Transaksi extends FrameTransaksi{
        ...
        public Transaksi() {
            try {
                ...
                String[] makananArr = comboMakanan.getSelectedItem().toString().split(":");
                this.id_makanan = Integer.parseInt(makananArr[0]);
                this.nama_makanan = makananArr[1];
                this.harga_makanan = Integer.parseInt(makananArr[2]);

                String[] minumanArr = comboMinuman.getSelectedItem().toString().split(":");
                this.id_minuman = Integer.parseInt(minumanArr[0]);
                this.nama_minuman = minumanArr[1];
                this.harga_minuman = Integer.parseInt(minumanArr[2]);

                String[] kasirArr = comboKasir.getSelectedItem().toString().split(":");
                this.id_user = Integer.parseInt(kasirArr[0]);
                this.username = kasirArr[1];

                String[] mejaArr = comboMeja.getSelectedItem().toString().split(":");
                this.id_meja = Integer.parseInt(mejaArr[0]);
                this.lokasi = mejaArr[1];
    ...
    public void refreshComboMakanan() {
        try {
            this.stat = k.getCon().prepareCall("SELECT * FROM makanan WHERE status='Tersedia'");
            this.rs = this.stat.executeQuery();
            while (rs.next()) {
    ...
    public void refreshComboMinuman() {
        try {
            this.stat = k.getCon().prepareCall("SELECT * FROM minuman WHERE status='Tersedia'");
            this.rs = this.stat.executeQuery();
            while (rs.next()) {
